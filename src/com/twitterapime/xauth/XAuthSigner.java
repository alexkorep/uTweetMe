...
public final class XAuthSigner {
   ...
   ////////////
   public String getSignHeader(Token access,
      String i_method, String i_sanitizedURL) {
		OAuthParameters params = new OAuthParameters(consumerKey);
		params.put(XAuthConstants.TOKEN, access.getToken());
		//
		String method = StringUtil.encode(i_method /*req.getMethod()*/, "UTF-8");
		String url = StringUtil.encode(i_sanitizedURL /*req.getSanitizedURL()*/, "UTF-8");
		//
		//addParams(params, i_queryStringParams /*req.getQueryStringParams()*/);
		//addParams(params, i_bodyParameters /*req.getBodyParameters()*/);
		//
		String sortedParams =
			StringUtil.encode(
				params.getSortedEncodedParamsAsString(), "UTF-8");
		//
		String str = method + '&' + url + '&' + sortedParams;
